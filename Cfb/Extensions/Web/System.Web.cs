...
namespace Cfb.Extensions
{
	...
	/// </summary>
	public static class System_Web_Extensions
	{
		/// <summary>
		/// Redirects a client to a new URL with a query string collection. Specifies the new URL and whether execution of the current page should terminate.
		/// </summary>
		...
		/// <param name="queryString">The query string collection to pass to the target.</param>
		public static void Redirect(this HttpResponse response, string url, NameValueCollection queryString)
		{
			if ((queryString == null) || (queryString.Count == 0))
				response.Redirect(url);
			else
				response.Redirect(url, queryString.ToQueryString());
		}

		/// <summary>
		/// Redirects a client to a new URL with a query string. Specifies the new URL and whether execution of the current page should terminate.
		/// </summary>
		/// <param name="response">The <see cref="HttpResponse"/> object for sending data to a client.</param>
		/// <param name="url">The location of the target.</param>
		/// <param name="queryString">The query string to pass to the target.</param>
		public static void Redirect(this HttpResponse response, string url, string queryString)
		{
			response.Redirect(string.Format("{0}?{1}", url, queryString));
		}
		...
		/// <summary>
		/// Gets the HTTP MIME content type of a file.
		/// </summary>
		/// <param name="response">The <see cref="HttpResponse"/> object for sending data to a client.</param>
		/// <param name="path">The path to the file to examine.</param>
		/// <returns>The HTTP MIME content type of the file indicated.</returns>
		public static string GetContentType(this HttpResponse response, string path)
		{
